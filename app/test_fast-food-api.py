...
#red testa
def test_orders():
    """test to see if orders are created"""
...
def test_new_order():
    ...
    response = result.post('/api/v1/orders',\
    	               data=json.dumps(item_to_be_added), content_type='application/json')
...
def test_update():
    ...
    result = app.test_client()
    response = result.put('/api/v1/order',\
    	              data=json.dumps(item_to_be_added), content_type='application/json')
